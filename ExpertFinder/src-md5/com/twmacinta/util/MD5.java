 ...
 * @author ralph (Modified code to work with arbitrary input streams instead of files)
 */
...
public class MD5 {
    ...
    /**
     * Calculates and returns the hash of the contents of the given url.
     **/
    public static byte[] getHash (URL url) throws IOException {
        InputStream close_me = null;
        try {
            int buf_size = 65536;
            byte[] buf = new byte[(int) buf_size];
            MD5InputStream in = new MD5InputStream(url.openStream());
            close_me = in;
            while (in.read(buf) != -1);
            in.close();
            return in.hash();
